    ...
    <>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              ...
              <item.icon
                className={`size-4.25 shrink-0 ${isActive ? "text-indigo-300" : "text-slate-400 group-hover:text-slate-300"}`}
              />
              <span className="flex-1">{item.name}</span>
              {isActive && (
                <ChevronRightIcon className="size-3.5 text-indigo-500/50" />
              )}
            </Link>
          );
        })}
      ...
      <div className="p-3 border-t border-white/6">
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 transition-all duration-300">
